...
TNode* ExprSeqNode::translate(Codegen* g, bool cfunc)
{
    ...
    for (int k = 0; k < (int)exprs.size(); ++k)
    {
        ...
        TNode* p;
        p = d_new TNode(IR_ARG, nullptr, nullptr, k * 4);
...
ExprNode* UniExprNode::semant(Environ* e)
{
    ...
    if (ConstNode* c = expr->constNode())
    {
        ...
        if (sem_type == Type::int_type)
        {
            switch (op)
            {
            case '+': e = d_new IntConstNode(+c->intValue());
                break;
            case '-': e = d_new IntConstNode(-c->intValue());
                break;
            case ABS: e = d_new IntConstNode(c->intValue() >= 0 ? c->intValue() : -c->intValue());
                break;
        ...
        } else
        {
            switch (op)
            {
            case '+': e = d_new FloatConstNode(+c->floatValue());
                break;
            case '-': e = d_new FloatConstNode(-c->floatValue());
                break;
            case ABS: e = d_new FloatConstNode(c->floatValue() >= 0 ? c->floatValue() : -c->floatValue());
                break;
            case SGN: e = d_new FloatConstNode((float)(c->floatValue() > 0 ? 1 : (c->floatValue() < 0 ? -1 : 0)));
                break;
            }
        }
        delete this;
...
ExprNode* BinExprNode::semant(Environ* e)
{
    ...
    if (lc && rc)
    {
        ...
        switch (op)
        {
        case AND: expr = d_new IntConstNode(lc->intValue() & rc->intValue());
            break;
        case OR: expr = d_new IntConstNode(lc->intValue() | rc->intValue());
            break;
        case XOR: expr = d_new IntConstNode(lc->intValue() ^ rc->intValue());
            break;
        case SHL: expr = d_new IntConstNode(lc->intValue() << rc->intValue());
            break;
        case SHR: expr = d_new IntConstNode((unsigned)lc->intValue() >> rc->intValue());
            break;
        case SAR: expr = d_new IntConstNode(lc->intValue() >> rc->intValue());
            break;
        }
        delete this;
        return expr;
...
ExprNode* ArithExprNode::semant(Environ* e)
{
    ...
    if (lc && rc)
    {
        ...
        } else if (sem_type == Type::int_type)
        {
            switch (op)
            {
            case '+': expr = d_new IntConstNode(lc->intValue() + rc->intValue());
                break;
            case '-': expr = d_new IntConstNode(lc->intValue() - rc->intValue());
                break;
            case '*': expr = d_new IntConstNode(lc->intValue() * rc->intValue());
        ...
        } else
        {
            switch (op)
            {
            case '+': expr = d_new FloatConstNode(lc->floatValue() + rc->floatValue());
                break;
            case '-': expr = d_new FloatConstNode(lc->floatValue() - rc->floatValue());
                break;
            case '*': expr = d_new FloatConstNode(lc->floatValue() * rc->floatValue());
            ...
            case '/': expr = d_new FloatConstNode(lc->floatValue() / rc->floatValue());
                break;
            case MOD: expr = d_new FloatConstNode(fmod(lc->floatValue(), rc->floatValue()));
                break;
            case '^': expr = d_new FloatConstNode(pow(lc->floatValue(), rc->floatValue()));
                break;
            }
        }
        delete this;
...
ExprNode* RelExprNode::semant(Environ* e)
{
    ...
    if (lc && rc)
    {
        ...
        if (opType == Type::string_type)
        {
            switch (op)
            {
            case '<': expr = d_new IntConstNode(lc->stringValue() < rc->stringValue());
                break;
            case '=': expr = d_new IntConstNode(lc->stringValue() == rc->stringValue());
                break;
            case '>': expr = d_new IntConstNode(lc->stringValue() > rc->stringValue());
                break;
            case LE: expr = d_new IntConstNode(lc->stringValue() <= rc->stringValue());
                break;
            case NE: expr = d_new IntConstNode(lc->stringValue() != rc->stringValue());
                break;
        ...
        } else if (opType == Type::float_type)
        {
            switch (op)
            {
            case '<': expr = d_new IntConstNode(lc->floatValue() < rc->floatValue());
                break;
            case '=': expr = d_new IntConstNode(lc->floatValue() == rc->floatValue());
                break;
            case '>': expr = d_new IntConstNode(lc->floatValue() > rc->floatValue());
                break;
            case LE: expr = d_new IntConstNode(lc->floatValue() <= rc->floatValue());
                break;
            case NE: expr = d_new IntConstNode(lc->floatValue() != rc->floatValue());
                break;
        ...
        } else
        {
            ...
            case '>': expr = d_new IntConstNode(lc->intValue() > rc->intValue());
                break;
            case LE: expr = d_new IntConstNode(lc->intValue() <= rc->intValue());
                break;
            case NE: expr = d_new IntConstNode(lc->intValue() != rc->intValue());
                break;
            case GE: expr = d_new IntConstNode(lc->intValue() >= rc->intValue());
                break;
            }
        }
        delete this;
